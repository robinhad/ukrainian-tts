...
The process uses these four profiles:
...
3. `resemble_enhance` uses the complete Resemble Enhance model. It does not use
   the `denoise_only` mode. It uses the command defaults: NFE 64, midpoint
   solver, lambda 1.0, and tau 0.5.
4. `clearervoice` uses the default pretrained `MossFormer2_SE_48K` model.
...
- The directory has 40 enhanced output files and 40 metadata files.
- All 40 output files use PCM 24-bit encoding.
