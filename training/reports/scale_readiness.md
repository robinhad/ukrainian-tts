...
| 4.5M memory reserve | FAIL | Peak cached VRAM was 22.684 GiB, which leaves less than 10 percent free | `exp_multispeaker_full/tts_jets_uk_24k_multispeaker/train.log` | Use 3.8M for the long run |
| Long training | NOT RUN | The corrected 1k gate passed; the default batch setting is now 3.8M | `scripts/run_multispeaker_training.sh` | Resume to 25k |
...
All corrected data and one-epoch training gates are `PASS`. The 4.5M
memory-reserve gate is `FAIL`, so the 25k continuation uses 3.8M. Dmytro remains
an inference-only zero-shot target because no raw Dmytro corpus is available.
