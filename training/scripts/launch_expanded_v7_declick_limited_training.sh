...
setsid env TTS_EXP="$TTS_EXP" ALLOW_TRAINING_RESUME=1 \
    "${ROOT}/scripts/run_expanded_v7_declick_limited_training.sh" "$TARGET" \
    >"$LOG" 2>&1 &
train_pid=$!
...
for label in 25k 50k 75k 100k; do
    if [[ -s "${TTS_EXP}/milestones/${label}.pth" ]]; then
        TTS_EXP="$TTS_EXP" \
            "${ROOT}/scripts/evaluate_expanded_v7_declick_limited_milestone.sh" "$label"
    fi
done
echo "The v7 100K training and milestone evaluation chain is complete."
