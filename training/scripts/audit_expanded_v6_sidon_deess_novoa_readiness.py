...
def main() -> int:
    ...
    validation_path = (
        args.reports_root / "expanded_v6_sidon_deess_novoa_full_validation.json"
    )
    embedding_path = (
        args.reports_root / "expanded_v6_sidon_deess_novoa_full_hybrid_embeddings.json"
    )
    ...
    profile_ok = bool(len(frame)) and all(
        [
            ...
            frame["declicking_applied"].fillna(False).astype(bool).all(),
            ...
            not frame["loudness_normalization_applied"]
            .fillna(False)
            .astype(bool)
            .any(),
            frame["limiting_applied"].fillna(False).astype(bool).all(),
        ]
    )
    emb_counts = embedding.get("counts", {}) if embedding else {}
    archive_records = sum(
        int(x.get("records", 0)) for x in (embedding or {}).get("archives", {}).values()
    )
    token_hash = (
    ...
    gates = [
        ...
        gate(
            "Sidon processing profile",
            profile_ok,
            "Sidon, light de-essing, de-clicking, and peak limiting are on. Other enhancement steps are off.",
            manifest,
        ),
        gate(
            "Dataset validation",
        ...
        gate(
            ...
            bool(
                ...
                and emb_counts == {"clean": 37_078, "raw": 37_078}
                and embedding.get("reused_raw_vectors") == 37_078
                and embedding.get("recomputed_clean_vectors") == 37_078
