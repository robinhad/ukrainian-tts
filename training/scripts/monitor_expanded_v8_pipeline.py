#!/usr/bin/env python3
"""Write v8 phase, resource, progress, and Kyiv ETA records."""
...
def phase(processes: str, running: bool) -> str:
    ...
    if "evaluate_expanded_v8" in processes:
        return "milestone_evaluation"
    if "espnet2.bin.gan_tts_train --collect_stats true" in processes:
        return "statistics"
    if "run_expanded_v8_training_cascade_smoke" in processes:
...
def main() -> int:
    ...
    initial = result_count(args.root)
    while True:
        ...
        rate = max(processed - initial, 0) / elapsed
        phase_eta = None
        total_eta = None
        eta_basis = "not_available"
        prep = latest(args.root / f"reports/{NAME}_preparation_status.jsonl")
        training = latest(args.root / f"reports/training_status_{NAME}_100k.jsonl")
        if current_phase == "training_cascade_preprocessing":
            phase_eta = prep.get("eta_kyiv") if prep else None
            if phase_eta:
                total_eta = (datetime.fromisoformat(phase_eta) + timedelta(hours=27)).isoformat()
            eta_basis = "observed_file_rate_plus_27h_downstream_estimate"
        elif current_phase in {"manifests_embeddings_tokens_or_statistics", "statistics", "clean_speaker_embeddings"}:
            phase_eta = (now + timedelta(hours=3)).isoformat()
            total_eta = (now + timedelta(hours=29)).isoformat()
            ...
            total_eta = (now + timedelta(hours=25)).isoformat()
            eta_basis = "prior_pipeline_runtime_estimate"
            ...
            total_eta = phase_eta
            eta_basis = "prior_evaluation_runtime_estimate"
        elif current_phase == "initializing_or_transition":
            phase_eta = (now + timedelta(minutes=15)).isoformat()
            total_eta = (now + timedelta(hours=34)).isoformat()
        ...
        record = {
            ...
            "observed_rate_files_per_second": round(rate, 5),
