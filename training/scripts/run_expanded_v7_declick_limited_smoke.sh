...
TTS_EXP=${TTS_EXP:-"${ROOT}/exp_${NAME}_smoke/tts_jets_uk_24k_${NAME}_from_v6e94_smoke"}
...
if json.loads(path.read_text(encoding="utf-8")).get("status") != "PASS":
    raise SystemExit("The v7 readiness report is not PASS.")
PY
...
echo "The v7 smoke gate is complete."
