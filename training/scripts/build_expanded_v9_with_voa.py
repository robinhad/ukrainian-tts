#!/usr/bin/env python3
"""Build the VOA-inclusive v9 manifests from completed cascade outputs."""
...
def main() -> int:
    ...
    raw = raw[raw["utterance_id"].astype(str).isin(expected)].copy()
    for label, frame in (("base", base), ("raw", raw), ("results", results)):
        if "utterance_id" not in frame or frame["utterance_id"].duplicated().any():
            raise SystemExit(f"The {label} data has invalid utterance IDs.")
    if len(base) != EXPECTED_RECORDS or len(raw) != EXPECTED_RECORDS:
        raise SystemExit("The v9 input record count is incorrect.")
    if set(raw["utterance_id"].astype(str)) != expected:
        raise SystemExit("The pre-enhancement input does not cover the v9 corpus.")
    if set(results["utterance_id"].astype(str)) != expected:
        raise SystemExit("The processing results do not cover the v9 corpus.")
    ...
    frame["preprocessing_profile"] = NAME
    ...
    voa_records = int(
        frame["source_id"].astype(str).str.contains("voa", case=False).sum()
    )
    if voa_records != EXPECTED_VOA_RECORDS:
        raise SystemExit(f"The v9 corpus has {voa_records} VOA records.")
    if not frame["sample_rate"].astype(int).eq(24_000).all():
        raise SystemExit("The v9 corpus has a non-24-kHz record.")
    if not frame["channels"].astype(int).eq(1).all():
        raise SystemExit("The v9 corpus has a non-mono record.")
    if not frame["format"].astype(str).eq("WAV/PCM_24").all():
        raise SystemExit("The v9 corpus has a non-PCM24 record.")
    invalid_files = [
        path
        for path in frame["audio_path"]
        if Path(str(path)).is_symlink() or not Path(str(path)).is_file()
    ]
    if invalid_files:
        raise SystemExit("The v9 corpus has a missing file or a symbolic link.")
